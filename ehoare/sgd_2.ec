...
op grad (f : Rn.vector -> real) (x : Rn.vector) : Rn.vector.

axiom gradlin: forall (f g : Rn.vector -> real) (p q : real) (x : Rn.vector),
grad (fun v => p * (f v) + q * (g v)) x = p ** grad f x + q ** grad g x.

op islip (l : real) (f : Rn.vector -> real) : bool.

axiom lipfunc : forall (l : real) (f : Rn.vector -> real) (x y : Rn.vector), islip l f => norm (grad f x - grad f y) <= l * norm (x - y).

op isconvex (f : Rn.vector -> real) : bool.

axiom convexfunc : forall (f : Rn.vector -> real) (k : real) (x y : Rn.vector),
  isconvex f /\ 0.0 <= k /\ k <= 1.0 => f (k ** x + (1.0 - k) ** y) <= k * f x + (1.0 - k) * f y.

op gradestimate (i n : int) (variance : real) (f : Rn.vector -> real) (x : Rn.vector) : Rn.vector.

axiom unbiasedEstim : forall  (n : int) (k) (variance : real) (f : Rn.vector -> real) (x : Rn.vector),
  Ep [1.. n] (fun i => ((gradestimate i n variance f x).[k])%xr) = ((grad f x).[k])%xr.

axiom bddVar : forall  (n : int) (variance : real) (f : Rn.vector -> real) (x : Rn.vector),
  Ep [1.. n] (fun i => ((norm (gradestimate i n variance f x))^2)%xr)  <= (variance + (norm (grad f x))^2)%xr.

op objectiveFun (x : Rn.vector) : real.
axiom posObjFun : forall (x : Rn.vector), 0.0 <= objectiveFun x.
...
op headvec (l : vector list) : vector.
axiom headv : forall (l :vector list) (v : vector), headvec (v :: l) = v.

op largeconstant : real.

op nonEmpty (l : vector list) : bool = if (l = []) then false else true.

op minarg (f : Rn.vector -> real) : Rn.vector.

axiom minimize : forall (f : Rn.vector -> real) (x : Rn.vector), f (minarg f) <= f x.

op lipcst : real.
axiom lipPos : 0.0 < lipcst.
axiom lipschitz : islip lipcst objectiveFun. 

op numIter : int.
axiom num: 0 < numIter.

op variance : real.
axiom vari:  0.0 <= variance.

op t : real.
axiom t_const : 0.0 < t /\ t <= 1.0/lipcst.

op m : int.
axiom m_const : 1 < m.

op x_min : Rn.vector.
axiom eqxmin : x_min = minarg objectiveFun.

lemma potfunDec : forall (potential : real) (v : Rn.vector),
Ep [1..m]
  (fun (x : int) =>
     (potential +
      objectiveFun
        (v -
         t ** gradestimate x m variance objectiveFun (v)) -
      (objectiveFun x_min +
       (norm (v - x_min) ^ 2 -
        norm
          (v -
...
var x : int  
  ...
  l <- [x0];
   while (i < numIter)
    {y <- headvec l;
      x <$ [1..m];
      v <- gradestimate x m variance objectiveFun y;
      y' <- y - t ** v;
      l <- y' :: l;
      i <- i + 1;
       potential <- potential + objectiveFun (y') -(
objectiveFun (x_min) + 
(1.0/(2.0*t))* ((norm (y - x_min))^2  - (norm (y' - x_min))^2 + t*variance));
      
}}}.

ehoare sgdbound : Sgd.f : (Sgd.potential = largeconstant) `|` (Sgd.potential)%xr ==> (Sgd.potential)%xr. proof. proc. seq 2 : ((Sgd.i = 0 /\ Sgd.l = [Sgd.x0] /\
   Sgd.potential = largeconstant) `|`
  Sgd.potential%xr). wp. skip. smt. while (Sgd.potential%xr). smt. wp. skip. progress. case (Sgd.i{hr} < numIter). move => H. simplify. apply potfunDec. smt. auto. progress. smt. qed. 
